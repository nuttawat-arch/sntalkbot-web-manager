...
checks={
 ...
 'config save uses live loopback apply before restart fallback':'bot_api_apply_config' in app and 'kind="config-restart"' in app and 'restart_required' in app,
...
'tenant cannot repoint verified TeamTalk identity':'TENANT_LOCKED_CONFIG_KEYS' in app and 'ผู้ใช้ทั่วไปไม่สามารถเปลี่ยน TeamTalk connection/login identity' in app and 'f.locked' in configtpl,
 ...
 'System remote update probes do not block initial HTML':'system_status(False, include_expensive=False)' in app and '/system/remote-status' in app and 'ThreadPoolExecutor(max_workers=4)' in app and "fetch('/system/remote-status'" in js and 'การตรวจรุ่นล่าสุดทำต่อเบื้องหลัง' in system_tpl,
 'SNTalkBot image overview is concise version-only':'id="remote-bot"' in system_tpl and 'Local digest:' not in system_tpl and 'Remote latest:' not in system_tpl and 'data.bot_remote' in js and 'local_image_digest' not in js and 'remote_image_digest' not in js,
 'central Telegram UI documents per-instance ownership':'จะใช้ token และ Chat ID ของ instance นั้นเท่านั้น' in telegramtpl and 'Telegram ส่วนกลางใช้เฉพาะ instance ที่ไม่ได้ตั้ง token ของตัวเอง' in telegramtpl,
 'System page uses accessible sub-tabs':'class="subtabs"' in system_tpl and 'aria-current="page"' in system_tpl and 'tab=webhook' in system_tpl,
 'Config page uses accessible category sub-tabs':'class="subtabs"' in configtpl and 'config_tabs' in configtpl and 'active_tab' in configtpl,
 ...
 'Manager/Full release notifications have explicit bulk enable action':'enable-release-notifications' in app and 'job_enable_release_notifications' in app and 'role not in ("manager", "full")' in app and 'bot_api_apply_config(path, changed_keys)' in app,
